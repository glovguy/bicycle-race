...
function drawWorld(ctx, allObjs) {
  ...
  allObjs.forEach((obj) => { drawObject(ctx, obj); });
}
exports.drawWorld = drawWorld;

function drawObject(ctx, obj) {
  if (obj.display.decay > 0 && obj.display.decay <= fadeOutFrames * physics.timeDel) {
    ctx.globalAlpha = obj.display.decay / (fadeOutFrames * physics.timeDel);
  }
...
function drawBall(obj) {
  rc.circle(obj.pos.x, obj.pos.y, obj.size*2, {
    ...
    strokeWidth: 1,
    roughness: 0.8*Math.abs(obj.vel.x / physics.maxHorizontalAirSpeed) + 1.8*Math.abs(obj.vel.y / physics.maxFlightClimbSpeed) + 0.4
  });
  if (obj.kineticState.rattled > 0) {
    ...
    obj.kineticState.rattled -= physics.timeDel;
  }
}
...
function drawEraseMarks(ctx, obj) {
